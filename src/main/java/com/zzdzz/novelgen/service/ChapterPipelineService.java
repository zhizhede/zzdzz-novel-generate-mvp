...
public class ChapterPipelineService {
    ...
    /** 向 SSE 订阅者推管线事件（无订阅者时静默）。 */
    private void emit(String stage, Map<String, Object> data) {
        sse.send(stage, data);
    }
    ...
    public boolean tryRunAsync(String novelTitle, int from, int to) {
        ...
        emit("run", Map.of("phase", "start", "novel", novelTitle, "from", from, "to", to));
        pipelineExecutor.submit(() -> {
            try {
                int ok = runChapters(novelTitle, from, to);
                lastMessage = "完成：通过 " + ok + " 章";
                emit("run", Map.of("phase", "done", "passed", ok, "total", to - from + 1));
            } catch (Exception e) {
                lastMessage = "异常：" + e.getMessage();
                emit("run", Map.of("phase", "error", "message", String.valueOf(e.getMessage())));
            } finally {
                running.set(false);
            }
        });
    ...
    private boolean runChapter(long novelId, int chapterNo, String approvalMode) {
        ChapterDO ch = outlineService.loadChapter(novelId, chapterNo);
        emit("chapter", Map.of("phase", "start", "chapterNo", chapterNo, "title", String.valueOf(ch.title())));

        // 1) AI 章纲（已物化则跳过——场景级断点续跑）
        if (sceneDAO.countByChapter(ch.id()) == 0) {
            emit("outline", Map.of("chapterNo", chapterNo, "phase", "start"));
            List<String> digests = packer.recentDigests(novelId, chapterNo, 3);
            outlineService.generate(novelId, ch, packer.world(novelId), packer.characters(novelId),
                    packer.foreshadowDirectives(novelId, chapterNo), digests,
                    packer.prevTail(novelId, chapterNo));
            emit("outline", Map.of("chapterNo", chapterNo, "phase", "done",
        ...
        for (var spec : specs) {
            ...
            if (done != null && "PASSED".equals(done.gateStatus())) {
                ...
                emit("scene", Map.of("chapterNo", chapterNo, "sceneNo", spec.sceneNo(),
            ...
            for (int round = 1; round <= 2 && !ok; round++) {
                ...
                emit("gate", Map.of("chapterNo", chapterNo, "sceneNo", spec.sceneNo(),
            ...
            emit("gate", Map.of("chapterNo", chapterNo, "sceneNo", spec.sceneNo(), "passed", ok));
        ...
        if (passedScenes < specs.size()) {
            ...
            emit("chapter", Map.of("phase", "failed", "chapterNo", chapterNo,
        ...
        if (!gateService.checkChapter(novelId, ch.id(), chapterNo, fullText, ch.budgetMin(), ch.budgetMax())) {
            emit("chapter_gate", Map.of("chapterNo", chapterNo, "passed", false));
            ...
            if (stored != null && !stored.equals(fullText)
                ...
                emit("revise", Map.of("chapterNo", chapterNo, "phase", "reuse", "chars", stored.length()));
                fullText = stored;
                chapterDAO.saveFullText(ch.id(), fullText);
            } else {
                log.warn("第 {} 章章级门禁未过，带意见修订一轮", chapterNo);
                emit("revise", Map.of("chapterNo", chapterNo, "phase", "start"));
                ...
                if (revised == null || revised.length() < fullText.length() * 0.6) {
                    ...
                    emit("chapter", Map.of("phase", "failed", "chapterNo", chapterNo, "reason", "修订稿长度异常"));
                    chapterDAO.updateStatus(ch.id(), "FAILED");
                    return false;
                }
                ...
                if (!gateService.checkChapter(novelId, ch.id(), chapterNo, fullText, ch.budgetMin(), ch.budgetMax())) {
                    ...
                    emit("chapter", Map.of("phase", "failed", "chapterNo", chapterNo, "reason", "修订后门禁仍未过"));
                    chapterDAO.updateStatus(ch.id(), "FAILED");
                    return false;
                }
            }
        ...
        if ("manual".equals(approvalMode)) {
            ...
            emit("approve", Map.of("chapterNo", chapterNo, "phase", "pending"));
            return true;
        }

        // 5) digest
        digestService.digest(novelId, ch.id(), chapterNo, fullText);
        emit("digest", Map.of("chapterNo", chapterNo, "phase", "done"));
        emit("chapter", Map.of("phase", "done", "chapterNo", chapterNo, "chars", fullText.length()));
        return true;
    }
}
