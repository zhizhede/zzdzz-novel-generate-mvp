...
public interface ChapterStepDataService extends IService<ChapterStepDO> {
    ...
    List<ChapterStepDO> listByChapter(long chapterId);

    /** 最近一条非成功终态行（FAILED/INTERRUPTED），一屏 failureBrief 用；无则 null。 */
    ChapterStepDO latestUnsuccessful(long chapterId);

    /** 某章某步是否存在 RUNNING 行（digest 进行中判定等）。 */
    boolean hasRunning(long chapterId, String step);

    /** 某书某章最新 RUNNING 行（工作台「当前阶段」读模型）；无则空。 */
    java.util.Optional<ChapterStepDO> latestRunningByNovelAndChapterNo(long novelId, int chapterNo);
