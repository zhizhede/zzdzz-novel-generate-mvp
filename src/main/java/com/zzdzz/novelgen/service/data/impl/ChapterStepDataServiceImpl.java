...
public class ChapterStepDataServiceImpl extends ServiceImpl<ChapterStepMapper, ChapterStepDTO>
    ...
    private QueryWrapper<ChapterStepDTO> whereChapter(long chapterId) {
        return new QueryWrapper<ChapterStepDTO>()
    ...
    public Long start(long novelId, long chapterId, int chapterNo, String step, String subKey, int attempt) {
        ChapterStepDTO row = new ChapterStepDTO();
    ...
    public void finish(long stepId, String status, String detailJson) {
        ChapterStepDTO patch = new ChapterStepDTO();
        patch.setId(stepId);
    ...
    @Override
    public List<ChapterStepDTO> listByChapter(long chapterId) {
        return list(whereChapter(chapterId).orderByAsc("id"));
    }

    @Override
    public ChapterStepDTO latestUnsuccessful(long chapterId) {
        // 每个 (step, subKey) 取最新一行，再在其中找最近一次失败/中断——
        // 同一步骤后续已 DONE 的旧失败不算（如 digest 第一次撞唯一键、第二次成功）
        java.util.LinkedHashMap<String, ChapterStepDTO> latest = new java.util.LinkedHashMap<>();
        for (ChapterStepDTO row : listByChapter(chapterId)) {
    ...
    @Override
    public java.util.Optional<ChapterStepDTO> latestRunningByNovelAndChapterNo(long novelId, int chapterNo) {
        return java.util.Optional.ofNullable(getOne(new QueryWrapper<ChapterStepDTO>()
                .eq("novel_id", novelId)
    ...
    public void finishRunningInterrupted(long chapterId) {
        update(new com.baomidou.mybatisplus.core.conditions.update.UpdateWrapper<ChapterStepDTO>()
                .eq("chapter_id", chapterId)
                .eq("status", StepStatus.RUNNING.wire())
                .eq("is_deleted", false)
